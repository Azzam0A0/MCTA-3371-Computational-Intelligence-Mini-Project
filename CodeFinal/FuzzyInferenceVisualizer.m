...
function fis = create_fuzzy_navigation_system()
    ...
    fis = addMF(fis, 'ObstacleDistance', 'trimf', [0 0 4], 'Name', 'VeryClose');
    fis = addMF(fis, 'ObstacleDistance', 'trimf', [3 5 7], 'Name', 'Close');
    fis = addMF(fis, 'ObstacleDistance', 'trimf', [6 10 10], 'Name', 'Safe');
    
    % OUTPUT: Movement Urgency (0-100)
    fis = addOutput(fis, [0 100], 'Name', 'MovementUrgency');
    fis = addMF(fis, 'MovementUrgency', 'trimf', [0 0 40], 'Name', 'Careful');
    fis = addMF(fis, 'MovementUrgency', 'trimf', [20 50 75], 'Name', 'Normal');
    fis = addMF(fis, 'MovementUrgency', 'trimf', [60 100 100], 'Name', 'Aggressive');
...
function visualize_membership_functions()
    ...
    veryclose = trimf(x2, [0 0 4]);
    plot(x2, veryclose, 'r', 'LineWidth', 2.5);
    hold on;
    
    % Close: trimf [1 3 5]
    close = trimf(x2, [3 5 7]);
    plot(x2, close, 'g', 'LineWidth', 2.5);
    
    % Safe: trimf [4 10 10]
    safe = trimf(x2, [6 10 10]);
